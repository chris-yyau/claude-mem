...
| `CLAUDE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, `openrouter`, or `opencode` |
...
Use this provider when you have the local [`opencode`](https://github.com/sst/opencode) CLI installed — claude-mem shells out to `opencode run --format json` for compression, so no separate API key is needed (OpenCode handles auth via whatever provider it's already configured to use).

| Setting                                  | Default     | Description                                                                                |
|------------------------------------------|-------------|--------------------------------------------------------------------------------------------|
| `CLAUDE_MEM_OPENCODE_MODEL`              | _(empty)_   | Model in `provider/model` form (e.g. `opencode-go/glm-5.1`). Empty = opencode's default.   |
| `CLAUDE_MEM_OPENCODE_MAX_TOKENS`         | `100000`    | Estimated-token warning threshold. Logged when prompts exceed it.                          |
| `CLAUDE_MEM_OPENCODE_SKIP_PERMISSIONS`   | `false`     | Opt-in: pass `--dangerously-skip-permissions` to opencode. Off by default.                 |

The interactive installer runs `opencode models` to enumerate available models, sorts cheap/fast candidates (haiku, flash, lite, nano, mini, free) to the top with a star marker, and falls back to a free-text input if `opencode` isn't on PATH.
